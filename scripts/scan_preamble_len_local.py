...
def main() -> None:
    ...
    ted_data_clip = 0.3
    mu_fse = 1e-5
    preamble_grid = [50, 75, 100, 128, 150, 200, 256, 320]
    ...
    rows = []
    for preamble_len in preamble_grid:
    ...
    rows.sort()
    print(
        "Candidates (ber, tau_std, preamble_len, errors, shift, tau_final, tau_mean, tau_acq):"
    )
    for row in rows:
        print(row)
