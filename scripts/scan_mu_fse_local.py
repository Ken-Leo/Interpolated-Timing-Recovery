...
def main() -> None:
    ...
    beta = 0.00013
    ted_data_clip = 0.3
    mu_grid = [0.0, 1e-6, 3e-6, 1e-5, 3e-5, 1e-4]
    ...
    for mu_fse in mu_grid:
        ...
        a_hat, tau_hat, fse_mse = opsps.decode(
            r_filtered,
            ground_truth=None,
        ...
        rows.append(
            (
                ...
                (
                    float(np.mean(fse_mse[-50:]))
                    if len(fse_mse) >= 50
                    else float(np.mean(fse_mse))
                ),
            )
        )
    ...
    rows.sort()
    print(
        "Candidates (ber, tau_std, mu_fse, errors, shift, tau_final, tau_acq, mse_tail):"
    )
    for row in rows:
        print(row)
